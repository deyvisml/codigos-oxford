...
@push('headers')
    <meta name="description"
        content="Adquiere tu código para {{ $product->name }} ({{ $product->isbn }}), edición {{ $product->edition }}, serie {{ $serie->name }}, precio {{ number_format($product->price_usd, 2) }} USD, duración {{ $product->licence_length }}">
        ...
        <div class="my-8 mb-20">
            <div class="flex flex-wrap items-center gap-x-2 text-sm sm:text-base">
                ...
                <a href="{{ route('series.show', ['category' => $category, 'serie' => $serie]) }}"
                    title="{{ $serie->name }}" class="text-sky-800 hover:underline">
                    <h2>{{ $serie->name }}</h2>
                </a>
                ...
                <a href="{{ route('series.show', ['category' => $category, 'serie' => $serie, 'level' => $product->level->id]) }}"
                    title="{{ $product->level->name }}" class="text-sky-800 hover:underline">
                    <p>{{ $product->level->name }}</p>
                </a>
            </div>
            ...
            <div class="flex flex-wrap justify-around items-start my-5 py-5">
                <div class="flex justify-center items-center p-5 border rounded md:w-52 h-52">
                    <img src="{{ asset('images/products/' . basename($product->oup_image_url)) }}" alt="imagen producto"
                        title="{{ $product->name }}" class="max-w-[150px] h-full object-contain">
                </div>
                ...
                <div class="order-3 md:order-2 md:m-5 mx-2 my-5 md:my-0 w-full md:w-6/12">
                    ...
                    <ul class="bg-white my-5 rounded">
                        ...
                        <li class="flex border-gray-300 border-b">
                            ...
                            <p class="bg-gray-50 p-3 w-2/3 text-sm">
                                {{ str_replace('months', 'meses', $product->license_length) }}</p>
                        </li>
                        <li class="flex border-gray-300 border-b">
                ...
                <div
                    ...
                    <p class="font-semibold text-3xl">{{ number_format($product->price_usd, 2) }} <span
                            class="text-xl">USD</span></p>
                    ...
                    <div class="flex justify-center items-center mt-3 w-full h-9">
                        <img src="{{ asset('images/payment-methods.png') }}" alt="imagen formas de pago"
                            title="formas de pago" class="max-h-full object-contain">
                    </div>

                    <a class="block mt-5 py-2 w-full text-sm"
                        href="https://api.whatsapp.com/send?phone=51938544411&amp;text=Hola, tengo algunas consultas sobre el producto: {{ urlencode(url()->full()) }}"
                        target="_blank" title="consultas del producto">
                        Consultas sobre el producto? <span class="font-semibold text-green-600">Click aquí <i
                    ...
                    <div data-te-modal-init
                        ...
                        id="payment_modal" tabindex="-1" aria-labelledby="payment_modal" aria-modal="true"
                        role="dialog">
                                ...
                                <li class="w-full splide__slide">
                                    <div
                                        ...
                                        <a href="{{ route('products.index', ['product' => $similar_product]) }}"
                                            title="{{ $similar_product->name }}" class="block p-3 cursor-pointer">
                                            <div class="flex flex-col justify-center items-center text-center">
                                                <div class="flex justify-center items-center w-full h-44">
                                                    <img src="{{ asset('images/products/' . basename($similar_product->oup_image_url)) }}"
                                                        alt="imagen producto" title="{{ $similar_product->name }}"
                                                        class="max-w-[140px] h-full object-contain">
                                                </div>
                                                ...
                                                <div class="w-full text-start">
                                                    ...
                                                    <p
                                                        ...
                                                        ISBN: <span
                                                            class="font-normal">{{ $similar_product->isbn }}</span>
                                                    </p>
                ...
                createOrder: function(data, actions) {
                    ...
                    return actions.order.create({
                        purchase_units: [{
                            ...
                            "amount": {
                                "currency_code": "USD",
                                "value": (Number(total_price_item) + Number(total_price_tax))
                                    .toFixed(2),
                                "breakdown": {
                                    "item_total": {
                                        "currency_code": "USD",
